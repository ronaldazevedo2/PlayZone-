...
class _TelaStatusArenaEstado extends State<TelaStatusArena> {
  ...
  void _atualizarStatus() {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text("Atualizando status da arena..."),
  ...
  Widget build(BuildContext context) {
    ...
    final totalDentro = clientes.where((c) => c.statusAcesso == TipoStatusAcesso.dentro).length;
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          _construirItemLegenda(const Color(0xFF0B7F38), "Dentro da Arena", totalDentro),
                          const SizedBox(height: 8),
                          _construirItemLegenda(const Color(0xFFCBD5E1), "Fora da Arena", totalFora),
                          const SizedBox(height: 8),
                          _construirItemLegenda(const Color(0xFF09398E), "Quadras em Uso", quadrasOcupadas.length),
                        ],
                      ),
                    ),
                  ],
  ...
  Widget _construirAbaQuadras(Set<String> quadrasOcupadas) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        final ocupada = quadrasOcupadas.contains(nome) || (horario != "Livre" && index % 2 == 0); // Alguma simulação de ocupação fixa combinada
          ...
          child: Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color: ocupada ? const Color(0xFFDCFCE7) : const Color(0xFFF1F5F9),
                  shape: BoxShape.circle,
                ),
                child: Icon(
                  Icons.sports_soccer,
                  color: ocupada ? const Color(0xFF0B7F38) : const Color(0xFF64748B),
                  size: 20,
                ),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      horario == "Livre" && ocupada ? "Ocupação imediata" : horario,
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: ocupada ? const Color(0xFF0B7F38) : const Color(0xFFF1F5F9),
                  border: Border.all(color: ocupada ? const Color(0xFF0B7F38) : const Color(0xFFCBD5E1)),
  ...
  Widget _construirAbaJogadores(List<ClienteArena> clientes) {
    final jogadoresDentro = clientes.where((c) => c.statusAcesso == TipoStatusAcesso.dentro).toList();
    final jogadoresFora = clientes.where((c) => c.statusAcesso != TipoStatusAcesso.dentro).toList();
      ...
      children: [
        ...
        Row(
          ...
          children: [
            const Text(
              "Dentro da Arena",
              style: TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: Color(0xFF09398E)),
            ),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
              decoration: BoxDecoration(color: const Color(0xFF0B7F38).withOpacity(0.1), borderRadius: BorderRadius.circular(12)),
              child: Text(
                "${jogadoresDentro.length} ativos",
                style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Color(0xFF0B7F38)),
              ),
            ),
          ],
        ),
        ...
        if (jogadoresDentro.isEmpty)
          const Center(
            child: Padding(
              ...
              child: Text("Nenhum jogador dentro da arena no momento.", style: TextStyle(color: Color(0xFF64748B), fontSize: 13)),
            ),
          )
        else
          ListView.separated(
        ...
        if (jogadoresFora.isEmpty)
          const Center(
            child: Padding(
              padding: EdgeInsets.symmetric(vertical: 16.0),
              child: Text("Todos os jogadores estão dentro.", style: TextStyle(color: Color(0xFF64748B), fontSize: 13)),
            ),
          )
        else
          ListView.separated(
    ...
    return GestureDetector(
      onTap: () {
        Navigator.of(context).push(
          MaterialPageRoute(builder: (context) => TelaPerfilDetalhado(cliente: jogador)),
        );
      },
      child: Container(
          ...
          children: [
            ...
            ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: estaDentro ? Colors.red.shade50 : const Color(0xFF0B7F38),
...
class PintorOcupacaoCircular extends CustomPainter {
  ...
  bool shouldRepaint(covariant PintorOcupacaoCircular oldDelegate) {
    return oldDelegate.valorPreenchido != valorPreenchido || oldDelegate.valorTotal != valorTotal;
  }
}
